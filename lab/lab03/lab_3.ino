...
ISR(TIMER1_COMPA_vect) {
  ...
  // count_5s++;
  // if (count_5s % 5 == 0) {
  //   Serial.println("Trece timpul...");
  // }

  // Task1: Pulodagaita
  count_5s += 0.8; 
  if (count_5s - old > 5) {
    Serial.println("Schimb-o");
    normal = ~normal;
    old = count_5s;
  }
}
...
void configure_timer1(void) {
  Serial.begin(9600);
  // Task0: Afisati un mesaj o data la 5s
  TCCR1A = 0;
  TCCR1B = 0;
  TCNT1 = 0;
  OCR1A = 50000;
...
void loop() {
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    if (normal) {
      ...
      tone(buzzer, melody[thisNote]*2, noteDuration/2);
